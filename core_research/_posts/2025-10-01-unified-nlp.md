...
Our ultimate goal is to automatically ensure all data is right and to achieve a 100% accurate training dataset for any specific task without human intervention. For noisy data that requires LLM relabeling, if the label corrected by the LLM is neither equal (or similar) to LLM's previous output nor to our own trained model's output, we must manually determine the final label.
