...
Our goal is to ensure all data is right and to achieve a 100% accurate training dataset. For noisy data that requires relabeling, if the label corrected by the LLM is neither equal (or similar) to LLM's previous output nor to our own trained model's output, we must manually determine the final label.
