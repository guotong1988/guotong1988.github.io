...
**Step-2**. Re-set the labeling rule/knowledge for human. Then the knowledge-refreshed human re-label the training data that is recognised by badcase patterns of Step-1. When re-labeling, we **can not** provide the results of the last annotation and model prediction as references. Because if the model predicted result is wrong for a new evaluation/test data, then the new evaluation/test data's most similar data in the training dataset must be wrong. **During the relabeling process, we treat the human-annotated label from the test dataset as the preferred label. However, due to the limited accuracy of the similarity search algorithm, this is merely a suggested label, not a definitive one.** Then we train to get a new version of current deep model.
