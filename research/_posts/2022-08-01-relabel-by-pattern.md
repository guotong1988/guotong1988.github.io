...
The similar search method can also apply to quality inspection of the human-labeled dataset: We get a human-labeled dataset, and then we check and find a small set of error-labeled data. Then we can search the similar data of error data for human to correct them, so that it can reduce the range of dataset to check and correct.
