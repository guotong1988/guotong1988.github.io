...
Based on the good performance of previous works \cite{ref1} \cite{ref2} that have been verified on human-labeled dataset. We further apply the find-noise idea to dataset generated by user behavior and CTR task. The experiment result shows our method improves the offline AUC. Also, noise data is not the low-frequency user data. We will verify our idea on online performance in the future.  
